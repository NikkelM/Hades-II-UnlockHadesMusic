...
-- If Zagreus Journey is not installed, we add the songs with these requirements and costs
local defaultSongWorldUpgradeData = {
	ModsNikkelMUnlockHadesMusicMusicPlayerMainThemeMusicPlayer = {
...
-- If Zagreus Journey is installed, use alternate unlock conditions and costs
local zagreusJourneyWorldUpgradeData = {
	ModsNikkelMUnlockHadesMusicMusicPlayerMainThemeMusicPlayer = {
		Name = "ModsNikkelMUnlockHadesMusicMusicPlayerMainThemeMusicPlayer",
		InheritFrom = { "ModsNikkelMUnlockHadesMusicDefaultSongItem" },
		TrackName = "{2c36bb06-113d-4c63-8c35-ed21f4eb94ae}",
...
-- Make sure the inherited fields are applied properly
-- Then, depending on if Zagreus Journey is installed or not, choose the table to insert the new song data into the WorldUpgradeData table from
-- Since we do this on every game start/mod load, this will dynamically adapt if the player installs/uninstalls Zagreus Journey while having this mod installed
-- Unlocked songs stay unlocked either way, but non-unlocked, visible songs may disappear/reappear from the music player shop depending on the mod setup
local mods = rom.mods
local zagreusJourneyActive = mods["NikkelM-Zagreus_Journey"]

if zagreusJourneyActive then
	for songName, songData in pairs(zagreusJourneyWorldUpgradeData) do
		game.ProcessDataInheritance(songData, game.WorldUpgradeData)
		game.WorldUpgradeData[songName] = songData
	end
else
	for songName, songData in pairs(defaultSongWorldUpgradeData) do
		game.ProcessDataInheritance(songData, game.WorldUpgradeData)
		game.WorldUpgradeData[songName] = songData
	end
end
